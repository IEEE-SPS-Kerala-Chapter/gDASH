-- Every member of a team must be from the same college — teams aren't
-- cross-institution. Enforced here (not just client-side Zod) for the same
-- reason as the rest of submit_registration's checks: the anon key + RPC
-- name are publicly inspectable, so a direct API call could otherwise skip
-- the app's validation entirely.
--
-- Colleges are normalized (trimmed, case-insensitive, whitespace-collapsed)
-- before comparing, so "FISAT" vs "fisat" isn't flagged as a mismatch while
-- an actually different college still is.
...
begin
  ...
  v_leader_college_norm := lower(trim(regexp_replace(p_leader ->> 'college', '\s+', ' ', 'g')));
  ...
  loop
    ...
    if lower(trim(regexp_replace(v_member ->> 'college', '\s+', ' ', 'g'))) != v_leader_college_norm then
