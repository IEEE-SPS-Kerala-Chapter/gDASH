-- Closes a gap left open when leader email verification (Google OAuth /
-- email magic link, see lib/config.ts's LEADER_VERIFICATION_ENABLED) was
-- added: that check only ever lived in app/actions/registration.ts, which
-- resolves the leader's email from the caller's Supabase Auth session
-- instead of trusting the client payload. submit_registration() itself
-- never knew that feature existed — it just inserted whatever email was in
-- p_leader, so anyone calling this RPC directly (same anon key the site's
-- own JS bundle already exposes) could submit a team under a completely
-- unverified leader email, no sign-in required.
--
-- Same reasoning security_hardening.sql already applied to field lengths
-- and formats, extended to identity: a direct POST bypasses the app
-- entirely, so the guarantee has to be enforced here too, not just in
-- TypeScript.
--
-- Requires a real signed-in Supabase Auth session whose email matches
-- p_leader.email (case-insensitive, matching team_members_email_unique_idx's
-- own lower(email) comparison elsewhere in this schema).
